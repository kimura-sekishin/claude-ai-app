...
class TestTaskService:
    async def test_create_task_with_valid_data(self, mock_repository):
        # Given: 準備
    ...
    async def test_create_task_with_empty_title_raises_error(self, mock_repository):
        # Given: 準備
        service = TaskService(mock_repository)
        invalid_data = {"title": ""}
...
1. **Lintチェック & フォーマット**
   - **ruff**
     ...
     - flake8・isort・blackを一本化し、設定ファイルを削減
     - 設定ファイル: `pyproject.toml` (`[tool.ruff]`セクション)

2. **型チェック**
   - **mypy**
     - Pythonの静的型チェッカー
     - `strict`モードで厳密な型安全性を検証
     - 設定ファイル: `pyproject.toml` (`[tool.mypy]`セクション)

3. **テスト実行**
   - **pytest** + **pytest-asyncio**
     - Pythonの標準的なテストフレームワーク
     - 非同期テストを`asyncio_mode = "auto"`でシームレスにサポート
     - カバレッジ測定（pytest-cov）と連携可能
