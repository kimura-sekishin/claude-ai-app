...
データモデルはPythonのdataclassまたはTypedDictで定義します。
...
```python
class TaskManager:
    async def create_task(self, data: CreateTaskData) -> Task:
        """タスクを作成する"""
        ...

    async def list_tasks(self, filter: FilterOptions | None = None) -> list[Task]:
        """タスク一覧を取得する"""
        ...

    async def update_task(self, id: str, data: UpdateTaskData) -> Task:
        """タスクを更新する"""
        ...
...
class PriorityEstimator:
    ...
    def _calculate_deadline_score(self, due_date: datetime | None) -> int:
        ...
        if days_remaining < 0:  return 100
        if days_remaining <= 3: return 90
        if days_remaining <= 7: return 70
        if days_remaining <= 14: return 50
        return 30
...
3. **Python型定義**: dataclassまたはTypedDictでデータモデルを明確に
